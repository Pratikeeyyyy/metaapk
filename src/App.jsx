...
function App() {
  ...
  const connectWallet = useCallback(async () => {
    try {
      ...
      const functions = Object.keys(contractInstance).filter(key => 
    ...
    try {
      ...
      for (let i = 0; i < totalCount; i++) {
        try {
          ...
          const shareAmountEth = parseFloat(ethers.formatEther(exp.shareamount));
          ...
          const statusText = ['⏳ Pending', '✅ Paid', '❌ Rejected', '⚠️ Bad Debt'][Number(exp.status)] || 'Unknown';
      ...
      setDebugInfo(prev => prev + ` | Loaded: ${expenseList.length} expenses`);
  ...
  }, [contract, isConnected, expenseName, paidBy, person1, person2, amount, status, location, provider, loadExpenses, getContractBalance]);
  ...
  }, [contract, isConnected, isCorrectNetwork, loadExpenses, getContractBalance, provider]);
      ...
      <div className="w-full max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          ...
          <div className="text-center text-sm text-slate-500 mt-3">
            <i className={`fas fa-circle text-xs ${isConnected && isCorrectNetwork ? 'text-green-500' : 'text-gray-400'}`}></i>
            <span>
              {isConnected && isCorrectNetwork 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-orange-50 p-3 rounded-lg">
                <label className="text-sm font-semibold text-orange-700">Who Paid?</label>
                <input
                  type="text"
              ...
              <div className="bg-green-50 p-3 rounded-lg">
                <p className="font-semibold text-green-700">💰 Split Preview:</p>
                <p className="text-xl font-bold text-green-600">
                  {splitAmount} ETH per person
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Total: {amount || '0'} ETH (split equally between payer + 2 participants = 3 people total)
                </p>
              </div>
              ...
              {showBadDebt && (
                <div className="bg-red-50 p-3 rounded-lg border-2 border-red-300 animate-fadeIn">
                  <label className="text-sm font-semibold text-red-700 flex items-center gap-2">
                    <i className="fas fa-exclamation-triangle"></i> Mark as Bad Debt
                  </label>
                  <select
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl shadow-xl p-6 text-white">
              ...
              <p className="text-sm opacity-90 mt-2">Total ETH locked in contract</p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              ...
              <div className="mt-3 space-y-3">
                ...
                <div className="flex justify-between items-center border-b border-gray-100 pb-2">
                  ...
                  <span className="font-bold text-green-600 text-lg">
                    {expenses.reduce((sum, exp) => sum + exp.amt, 0).toFixed(4)} ETH
                  </span>
                </div>
                <div className="flex justify-between items-center border-b border-gray-100 pb-2">
              ...
              {dataLoaded && isConnected && isCorrectNetwork && expenses.length === 0 && (
              ...
              <div className="mt-3 space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                {!isConnected || !isCorrectNetwork ? (
                  ...
                  badDebtors.map((debtor, index) => (
                    <div key={index} className="bg-white p-3 rounded-lg border border-red-200 hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-center">
                        <span className="font-semibold text-red-700">{debtor.name}</span>
                        <span className="text-sm font-bold text-red-600">{debtor.amount.toFixed(3)} ETH</span>
                      </div>
                      <div className="text-xs text-gray-500 mt-1">
                  ...
                  expenses.map((exp) => (
                    <div 
                      ...
                      className={`bg-gray-50 rounded-lg p-3 border mb-2 transition-all hover:shadow-md ${
                        exp.status === 3 ? 'border-red-400 bg-red-50' : 'border-gray-200'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        <div className="font-bold text-slate-800">{exp.expname}</div>
                        <span className={`text-xs px-2 py-1 rounded-full ${
                          exp.status === 0 ? 'bg-yellow-100 text-yellow-700' :
                          exp.status === 1 ? 'bg-green-100 text-green-700' :
                          exp.status === 2 ? 'bg-red-100 text-red-700' :
                          'bg-red-200 text-red-800'
                        }`}>
                          {exp.statusText}
                        </span>
                      </div>
                      <div className="text-sm text-slate-600 mt-1">💰 {exp.amt.toFixed(3)} ETH | 👤 Paid by: {exp.paidby}</div>
                      <div className="text-xs text-slate-500">👥 {exp.person1} & {exp.person2} owe {exp.shareAmount} ETH each</div>
                      <div className="text-xs text-slate-500">📍 {exp.paddress}</div>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="bg-blue-50 rounded-2xl p-4 border border-blue-200">
              ...
              <ul className="text-xs text-blue-700 space-y-1">
                <li>1️⃣ Enter expense details (Payer + 2 participants = 3 total people)</li>
